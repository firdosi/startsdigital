...
Drop raw original assets into the respective ignored local folders:

```
private-assets/
├── hero/                    <-- Drop original hero dashboard composite & analytics screenshots
├── services/                <-- Drop original creative content/editing & AI workflow stills
...
### A. Hero & Services Visuals
- **Hero Primary**: Real agency dashboard or campaign composite (`private-assets/hero/hero-primary-original.png`).
- **Hero Secondary**: Real analytics screenshot or campaign card (`private-assets/hero/hero-secondary-original.png`).
- **Services Pillar C**: Real short-form video editing or ad creative layout (`private-assets/services/content-ai-original.png`).

### B. Black Gold Fertilizer
- **Required**: Transparent vector SVG or transparent PNG logo (minimum 1000px on longest side, preserve aspect ratio).
...
### C. Wajib Livestock (Qurbani Campaign)
- **Required**: Transparent vector SVG or transparent PNG logo (minimum 1000px on longest side, preserve aspect ratio).
- **Required**: At least 1 real livestock photo, Qurbani campaign ad creative, or video still.
- **Required**: Written confirmation of public case-study display name.
...
- **Required**: Approved founder full name, role, and short biography paragraph.
...
2. **Review & Redact**: Inspect asset for sensitive client data and redact if necessary.
3. **Format & Optimize**: Convert images to WebP (`80-85%` quality, max width `1440px`) or sanitize SVG icons.
4. **Copy to Public**: Copy optimized file into `public/<category>/<brand-slug>/` or `public/hero/`.
